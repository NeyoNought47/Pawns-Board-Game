...
public class PawnsBoardModelTest {
  ...
  private AbstractPawnsBoardModel readOnlyModel;
  ...
  public void setUp() {
    ...
    redPlayer = new HumanPlayer("Red", redDeck, 5);
    bluePlayer = new HumanPlayer("Blue", blueDeck, 5);

    board = new Board(3, 5);

    model = new PawnsBoardModelImpl(board, redPlayer, bluePlayer, 5);

    readOnlyModel = new ReadOnlyPawnsBoardModelImpl(board, redPlayer, bluePlayer, 5);
  ...
  // TEST for read only model

  @Test
  public void testGetCardAt() {
    model.startGame();
    model.placeCard(0, 0, redPlayer.getHand().get(0));
    ICard card = readOnlyModel.getCardAt(0, 0);
    Assert.assertEquals("Guardian", card.getName());
  }

  @Test(expected = IllegalArgumentException.class)
  public void testGetCardAt_NoCard() {
    readOnlyModel.getCardAt(1, 1);
  }

  @Test
  public void testGetPlayerHand() {
    model.startGame();
    List<ICard> hand = readOnlyModel.getPlayerHand(redPlayer);
    Assert.assertEquals(5, hand.size());
    Assert.assertEquals("Guardian", hand.get(0).getName());
  }

  @Test
  public void testScoreAtRowAndCurrentScore() {
    model.startGame();
    Assert.assertEquals(0, readOnlyModel.scoreAtRow(0, redPlayer));
    Assert.assertEquals(0, readOnlyModel.scoreAtRow(0, bluePlayer));

    Assert.assertEquals(0, readOnlyModel.currentScore(redPlayer));
    Assert.assertEquals(0, readOnlyModel.currentScore(bluePlayer));

    model.placeCard(0, 0, redPlayer.getHand().get(0));
    model.placeCard(1, 4, bluePlayer.getHand().get(0));

    Assert.assertEquals(3, readOnlyModel.scoreAtRow(0, redPlayer));
    Assert.assertEquals(0, readOnlyModel.scoreAtRow(0, bluePlayer));
    Assert.assertEquals(3, readOnlyModel.scoreAtRow(1, bluePlayer));

    Assert.assertEquals(3, readOnlyModel.currentScore(redPlayer));
    Assert.assertEquals(3, readOnlyModel.currentScore(bluePlayer));

    model.placeCard(1,0,redPlayer.getHand().get(0));

    Assert.assertEquals(3, readOnlyModel.scoreAtRow(0, redPlayer));
    Assert.assertEquals(2, readOnlyModel.scoreAtRow(1, redPlayer));
    Assert.assertEquals(3, readOnlyModel.scoreAtRow(1, bluePlayer));

    // score doesn't change because blue leads row 1
    Assert.assertEquals(3, readOnlyModel.currentScore(redPlayer));
    Assert.assertEquals(3, readOnlyModel.currentScore(bluePlayer));
  }
  ...
  public void testGetGameState() {
    Assert.assertEquals(GameState.NOT_STARTED, readOnlyModel.getGameState());
    model.startGame();
    Assert.assertEquals(GameState.ONGOING, readOnlyModel.getGameState());
    model.passTurn();
    model.passTurn();
    Assert.assertEquals(GameState.TIE, readOnlyModel.getGameState());
  }

  @Test
  public void testGetBoardDimensions() {
    Assert.assertEquals(3, readOnlyModel.getBoard().getRows());
    Assert.assertEquals(5, readOnlyModel.getBoard().getCols());
  }

  @Test
  public void testIsRedTurn() {
    model.startGame();
    Assert.assertTrue(readOnlyModel.isRedTurn());
    model.passTurn();
    Assert.assertFalse(readOnlyModel.isRedTurn());
  ...
  public void testIsGameOver() {
    Assert.assertFalse(readOnlyModel.isGameOver());
    model.startGame();
    model.passTurn();
    model.passTurn();
    Assert.assertTrue(readOnlyModel.isGameOver());
  }

  @Test(expected = IllegalStateException.class)
  public void testDetermineWinner_GameNotOver() {
    readOnlyModel.determineWinner();
  }

  @Test
  public void testIsMoveLegal() {
    model.startGame();
    Assert.assertFalse(readOnlyModel.isMoveLegal(2, 2,
            redPlayer.getHand().get(0), true));
    Assert.assertFalse(readOnlyModel.isMoveLegal(-2, 0,
            redPlayer.getHand().get(0), true));
    Assert.assertFalse(readOnlyModel.isMoveLegal(0, -2,
            redPlayer.getHand().get(0), true));
    Assert.assertFalse(readOnlyModel.isMoveLegal(100, 0,
            redPlayer.getHand().get(0), true));
    Assert.assertFalse(readOnlyModel.isMoveLegal(0, 100,
            redPlayer.getHand().get(0), true));
    Assert.assertTrue(readOnlyModel.isMoveLegal(0, 0,
            redPlayer.getHand().get(0), true));

    model.placeCard(1, 0, redPlayer.getHand().get(0));
    Assert.assertFalse(readOnlyModel.isMoveLegal(0,1,
            redPlayer.getHand().get(1), true));
  }

  @Test
  public void testOutOfBounds() {
    Assert.assertTrue(readOnlyModel.outOfBounds(-1, 0));
    Assert.assertTrue(readOnlyModel.outOfBounds(0, -1));
    Assert.assertTrue(readOnlyModel.outOfBounds(100,0 ));
    Assert.assertTrue(readOnlyModel.outOfBounds(0, 100));
    Assert.assertFalse(readOnlyModel.outOfBounds(2,3));
  }

  @Test
  public void testGetPlayers() {
    Assert.assertEquals("Red", readOnlyModel.getRedPlayer().getName());
    Assert.assertEquals("Blue", readOnlyModel.getBluePlayer().getName());
  }
  ...
  public void testOutOfBound() {
    Assert.assertTrue(model.outOfBounds(-1, 0));
    Assert.assertTrue(model.outOfBounds(5, 2));
    Assert.assertTrue(model.outOfBounds(0, -1));
    Assert.assertTrue(model.outOfBounds(0, 5));
    Assert.assertFalse(model.outOfBounds(2, 2));
  }
  ...
  public void testGetInfluenceGrid() {
    model.startGame();
    InfluenceGrid grid = redPlayer.getHand().get(0).getInfluenceGrid();
    char[][] g = grid.getGrid();
  ...
  public void testDetermineWinner() {
    ...
    model.passTurn();
    Assert.assertEquals(GameState.RED_WINS, model.getGameState());
    Assert.assertEquals(GameState.RED_WINS, readOnlyModel.getGameState());
  }
  ...
  public void testGetSize() {
    Assert.assertEquals(35, redPlayer.getDeck().size());
    Assert.assertEquals(35, bluePlayer.getDeck().size());
  }
  ...
  public void testInvalidPlayerInitialization() {
    Deck testDeck = new Deck(new ArrayList<>());
    IPlayer player = new HumanPlayer("test", testDeck, 5);
  ...
  public void testGetDeck() {
    Deck deck = redPlayer.getDeck();
    Assert.assertEquals(35, deck.size());
  ...
  public void testInfluenceGrid() {
    InfluenceGrid grid = new InfluenceGrid(new String[]{
        "XXXXX",
        "XXIXX",
        "XICIX",
        "XXIXX",
